#define mctuple_sim_cxx
// The class definition in mctuple_sim.h has been generated automatically
...
//
// root> T->Process("mctuple_sim.C")
// root> T->Process("mctuple_sim.C","some options")
// root> T->Process("mctuple_sim.C+")
//
...
void mctuple_sim::Begin(TTree * /*tree*/)
{
...
void mctuple_sim::SlaveBegin(TTree * /*tree*/)
{
...
Bool_t mctuple_sim::Process(Long64_t entry)
{
   ...
   for (int i=0; i < ncoin; ++i) {
      ...
      double m3gmc = sqrt(sqr(momf[0][0] + momf[1][0] + momf[2][0]) -
                          sqr(momf[0][1] + momf[1][1] + momf[2][1]) -
                          sqr(momf[0][2] + momf[1][2] + momf[2][2]) -
                          sqr(momf[0][3] + momf[1][3] + momf[2][3]));
      double tabsmc = sqr(momf[0][1] + momf[1][1] + momf[2][1] - momi[0][1]) +
                      sqr(momf[0][2] + momf[1][2] + momf[2][2] - momi[0][2]) +
                      sqr(momf[0][3] + momf[1][3] + momf[2][3] - momi[0][3]) -
                      sqr(momf[0][0] + momf[1][0] + momf[2][0] - momi[0][0]);
      double phirecmc = atan2(momf[3][2], momf[3][1]);
      double phifwdmc = atan2(momf[0][2] + momf[1][2] + momf[2][2], 
                              momf[0][1] + momf[1][1] + momf[2][1]);
      double dphimc = (phifwdmc - phirecmc) * 180/3.1416 + 180;
      while (dphimc < -180)
         dphimc += 360;
      while (dphimc > +180)
         dphimc -= 360;
      ...
      TLorentzVector ppi0MC = GetPpi0HelicityMC(momi[0][0]);
#if MAKE_SKIM_TREE
      if (tskim == 0) {
         if (skimfile == 0)
            skimfile = new TFile("/tmp/proof/jonesrt/mctuple_sim_skim.root", "recreate");
         ...
         tskimmc = new TTree("skimmc", "omega->pi0,gamma mc event skim");
         tskimmc->Branch("runevent", &tskimrow.runevent, "runevent/D");
         tskimmc->Branch("Ebeam", &tskimrow.Ebeam, "Ebeam/D");
         tskimmc->Branch("Emiss", &tskimrow.Emiss, "Emiss/D");
         tskimmc->Branch("m3g", &tskimrow.m3g, "m3g/D");
         tskimmc->Branch("dphi", &tskimrow.dphi, "dphi/D");
         tskimmc->Branch("tabs", &tskimrow.tabs, "tabs/D");
         tskimmc->Branch("costhetahel", &tskimrow.costhetahel, "costhetahel/D");
         tskimmc->Branch("phihel", &tskimrow.phihel, "phihel/D");
         tskimmc->Branch("weight", &tskimrow.weight, "weight/D");
      }
      if (tagw != 0) {
         tskimrow.runevent = runno + eventno/1.0e7;
         tskimrow.Ebeam = momi[0][0];
         tskimrow.Emiss = momi[0][0] - momf[0][0] - momf[1][0] - momf[2][0];
         tskimrow.m3g = m3gmc;
         tskimrow.dphi = dphimc;
         tskimrow.tabs = tabsmc;
         tskimrow.costhetahel = ppi0MC.CosTheta();
         tskimrow.phihel = ppi0MC.Phi();
         tskimrow.weight = 1;
         tskimmc->Fill();
...
TLorentzVector mctuple_sim::GetPpi0HelicityMC(double Ebeam)
{
   ...
   for (int n=0; n < 3; ++n) {
      pgam[n].SetPxPyPzE(momf[n][1], momf[n][2], momf[n][3], momf[n][0]);
   }
   ...
   // Assign the final proton momentum.
   p4lab.rec.SetXYZT(momf[3][1], momf[3][2], momf[3][3], momf[3][0]);
   ...
   if (tkinemc == 0) {
      if (dumpfile == 0)
         dumpfile = new TFile("mctuple_sim_dump.root", "recreate");
      tkinemc = new TTree("kinemc", "3gamma mc event kinematics");
      tkinemc->Branch("Ebeam", &tkinerow.Ebeam, "Ebeam/D");
      tkinemc->Branch("plabfwd", tkinerow.plabfwd, "plabfwd[4]/D");
      tkinemc->Branch("plabpi0", tkinerow.plabpi0, "plabpi0[4]/D");
      tkinemc->Branch("plabbac", tkinerow.plabbac, "plabbac[4]/D");
      tkinemc->Branch("plabrec", tkinerow.plabrec, "plabrec[4]/D");
      tkinemc->Branch("plabini", tkinerow.plabini, "plabini[4]/D");
      tkinemc->Branch("plabtot", tkinerow.plabtot, "plabtot[4]/D");
      tkinemc->Branch("preafwd", tkinerow.preafwd, "preafwd[4]/D");
      tkinemc->Branch("preapi0", tkinerow.preapi0, "preapi0[4]/D");
      tkinemc->Branch("preabac", tkinerow.preabac, "preabac[4]/D");
      tkinemc->Branch("prearec", tkinerow.prearec, "prearec[4]/D");
      tkinemc->Branch("preaini", tkinerow.preaini, "preaini[4]/D");
      tkinemc->Branch("preatot", tkinerow.preatot, "preatot[4]/D");
      tkinemc->Branch("preabea", tkinerow.preabea, "preabea[4]/D");
      tkinemc->Branch("phelfwd", tkinerow.phelfwd, "phelfwd[4]/D");
      tkinemc->Branch("phelpi0", tkinerow.phelpi0, "phelpi0[4]/D");
      tkinemc->Branch("phelbac", tkinerow.phelbac, "phelbac[4]/D");
      tkinemc->Branch("phelrec", tkinerow.phelrec, "phelrec[4]/D");
      tkinemc->Branch("phelini", tkinerow.phelini, "phelini[4]/D");
      tkinemc->Branch("pheltot", tkinerow.pheltot, "pheltot[4]/D");
      tkinemc->Branch("phelbea", tkinerow.phelbea, "phelbea[4]/D");
      tkinemc->Branch("theta0", &tkinerow.theta0, "theta0/D");
      tkinemc->Branch("theta1", &tkinerow.theta1, "theta1/D");
      tkinemc->Branch("costhetahel", &tkinerow.costhetahel, "costhetahel/D");
      tkinemc->Branch("phihel", &tkinerow.phihel, "phihel/D");
   }
...
TLorentzVector mctuple_sim::GetPpi0Helicity(double Ebeam)
{
   // Boost momenta into the helicty frame and there find the 4-momentum
   // of the pi0 assuming the 3gamma final state pi0(2gamma),gamma. To
   // To solve the kinematics of the reaction, I need to know the energy
   ...
   if (tkine == 0) {
      ...
      tkine = new TTree("kinemc", "3gamma mc event kinematics");
...
void mctuple_sim::SlaveTerminate()
{
...
void mctuple_sim::Terminate()
{
   // The Terminate() function is the last function to be called during
   // a query. It always runs on the client, it can be used to present
   // the results graphically or save the results to file.

   TFile fout("mctuple_sim.root", "recreate");
