#define mctuple_gen_cxx
// The class definition in mctuple_gen.h has been generated automatically
...
//
// root> T->Process("mctuple_gen.C")
// root> T->Process("mctuple_gen.C","some options")
// root> T->Process("mctuple_gen.C+")
//

#define ASSUME_PURE_SIGNAL_MC 1
...
void mctuple_gen::Begin(TTree * /*tree*/)
{
...
void mctuple_gen::SlaveBegin(TTree * /*tree*/)
{
...
Bool_t mctuple_gen::Process(Long64_t entry)
{
   ...
   if (fChain == 0) 
      return kFALSE;
   int nb = fChain->GetEntry(entry);       //read all branches
   // omega selection cuts
   if (nf != 4 || kindf[0] != 1 || kindf[1] != 1 || kindf[2] != 1 || kindf[3] != 14)
   ...
   for (int i=0; i < 3; ++i) {
      if (momf[i][0] < 0.050)
         true; //return kFALSE; // cut 1
      for (int mu=0; mu < 4; ++mu) {
         ptot[mu] += momf[i][mu];
      }
      double pperp = sqrt(sqr(momf[i][1]) + sqr(momf[i][2]));
      double theta = atan2(pperp, momf[i][3]) * 180/3.1415926;
      double A = 0.13;
      double B = 1.033;
      if (momf[i][0] < 0.5 && momf[i][0] < B - A * theta)
         true; //return kFALSE; // cut 6
      for (int j=i+1; j < 3; ++j) {
         double p[4] = {0};
         for (int mu=0; mu < 4; ++mu) {
            p[mu] = momf[i][mu] + momf[j][mu];
         }
   ...
   if (ptot[0] < 3.0)
      true; //return kFALSE; // cut 2

   if (mleast < 0.10 || mleast > 0.18)
      true; //return kFALSE; // cut 8

#if !ASSUME_PURE_SIGNAL_MC
   if (nrec != 1)
      return kFALSE; // cut 3
   ...
   double dphi = (atan2(ptot[2], ptot[1]) - 
                  atan2(momf[3][2], momf[3][1])) * 180/3.1415926;
   dphi += 180;
   while (dphi < -180) 
      dphi += 360;
   while (dphi > +180) 
   ...
   for (int i=0; i < ncoin; ++i) {
...
#endif
      double Elost = momi[0][0] - ptot[0];
      double Emiss = Elost;
      double tabs = sqr(ptot[1] - momi[0][1]) +
                    sqr(ptot[2] - momi[0][2]) +
                    sqr(ptot[3] - momi[0][3]) -
                    sqr(ptot[0] - momi[0][0]);
      ...
      if (tskimmc == 0) {
         if (skimfile == 0)
            skimfile = new TFile("/tmp/proof/jonesrt/mctuple_gen_skim.root", "recreate");
      ...
      if (fabs(m3g - 0.8) < 0.1 &&
          ...
          fabs(dphi) < 50)
      {
         h2heli[nhist]->Fill(ppi0MC.CosTheta(), ppi0MC.Phi(), tagw);
      }
   //}
   return kTRUE;
}

TLorentzVector mctuple_gen::GetPpi0HelicityMC(double Ebeam)
{
   ...
   if (tkinemc == 0) {
      if (dumpfile == 0)
         dumpfile = new TFile("mctuple_gen_dump.root", "recreate");
...
void mctuple_gen::SlaveTerminate()
{
...
void mctuple_gen::Terminate()
{
   // The Terminate() function is the last function to be called during
   // a query. It always runs on the client, it can be used to present
   // the results graphically or save the results to file.

   TFile fout("mctuple_gen.root", "recreate");
