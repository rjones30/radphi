 ...
 * Revision History:
 *   $Log: evio.c,v $
 *   Revision 1.4  2014/01/06 04:55:00  radphi
 *   * data.c dataIO.c disIO.c evio.c evt_dataIO.c fitting.c tapeIO.c tapeIoctl.c
 *   	- small changes to get rid of compiler warnings [rtj]
 *
 *   Revision 1.3  2003/06/26 13:27:15  radphi
 *   - evio.c - fixed NULL to 0 (compiler warning) -rtj
 *
 *   Revision 1.2  2003/03/04 19:37:19  radphi
 *   - fixed a bug in tapeIO.c function tape_ff() which failed if two sequential
 *     ff calls are made between calls to tape_read() if MULTI_BUFFER_IO [rtj]
 *
 *   Revision 1.1  1997/01/31 16:35:37  lfcrob
 *   Initial revision
 *
...
static char rcsid[] = "$Id: evio.c,v 1.4 2014/01/06 04:55:00 radphi Exp $";
...
extern  void onmemory_swap(void *buffer);
extern  int  swapped_fread();
extern  void swapped_intcpy(char *des,char *source,int size);
extern  void swapped_memcpy(char *buffer,char *source,int size);

int evopen_(char *filename,char *flags,intptr_t *handle,int fnlen,int flen)
{
  char *fn, *fl;
  int status;
...
int evread_(intptr_t *handle,int *buffer,int *buflen)
{
...
int evwrite_(intptr_t *handle,int *buffer)
{
...
int evioctl_(intptr_t *handle,char *request,void *argp,int reqlen)
{
  char *req;
  int status;
 ...
 *****************************************************************/
int evOpenSearch(intptr_t handle, intptr_t *b_handle)
{
  EVFILE *a;
  EVBSEARCH *b;
 ...
 *******************************************************************/
int evSearch(intptr_t handle, intptr_t b_handle, int evn, int *buffer, int buflen, int *size)
{
  EVFILE    *a;
  EVBSEARCH *b;
  int       start,end, mid;
 ...
 **********************************************************************/
int evCloseSearch(intptr_t b_handle)
{
  EVBSEARCH *b;
