		  ...
		  const char itemname[], int length, int type, int table_size )
{
  ...
  if ( subsystem.list >= 0 )
    ret = map_find_pred(fd,subsystem_loc+SUBSYSTEM_LIST_OFFSET,
			     item.name,0,NULL_LOC,&tindex,&succ_loc,&pred_loc);
  else
    ret = map_find_pred(fd,NULL_LOC,item.name,
  ...
  if ( succ_loc != NULL_LOC )
    {
      status = map_read_item(&successor,sizeof successor,fd,succ_loc);
      if ( status != MAP_OK )
	{
	  map_close_map(fd);
	  return status;
  ...
  ret = map_write_table(table,sizeof(table_t)*item.table_length,fd,&item.table);
  free(table);
  if ( ret < 0 )
    {
      map_close_map(fd);
  ...
  ret = map_write_item(&item,sizeof item,fd,&item_loc);
  if ( ret < 0 )
    {
      map_close_map(fd);
      return ret;
  ...
  if ( subsystem.list >= 0 )
...

    {
      ...
      status = map_overwrite_pointer(&item_loc,sizeof item_loc,fd,pred_loc);
      if ( status != MAP_OK )
	{
	  map_close_map(fd);
	  return status;
	...
	{
	  status = map_read_nametable(nametable,subsystem.table_used*sizeof(nametable_t),
			              fd,subsystem.table);
	  if ( status != MAP_OK )
	    {
	      free(nametable);
	...
	memmove(&nametable[tindex+2],&nametable[tindex+1],
      ...
      if ( subsystem.table_used <= subsystem.table_length )
	{
	  status = map_overwrite_nametable(nametable,
      ...
      else
	{
	  ...
	  ret = map_write_nametable(nametable,
      ...
      free(nametable);
      status = map_overwrite_subsystem(&subsystem,sizeof subsystem,fd,subsystem_loc);
      if ( status != MAP_OK )
	{
	  map_close_map(fd);
	  return status;
