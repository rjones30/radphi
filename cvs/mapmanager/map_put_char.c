 ...
 
{
  ...
  if ( suc_time == header.time )
	...
	{  
	  ...
	  status = map_read_arrayheader(&suc_header,sizeof suc_header,fd,header.next);
	  if ( status != MAP_OK )
	    {
	      map_close_map(fd);
	      return status;
  ...
  status = map_write_char(array,header.length*sizeof(char),fd,&dummy);
  if ( status < 0 )
    {
      map_close_map(fd);
      return status;
  ...
  if ( item.list < 0 )
      ...
      else
	{
	  ...
	  status = map_write_table(table,item.table_length*sizeof(table_t),fd,
				   &item.table);
	  if ( status < 0 )
	    {
	      free(table);
