     ...
     
{
  ...
  ret = map_write_nametable(table,header.table_length*sizeof(nametable_t),fd,&header.table);
  free(table);
  if ( ret < 0 )
	{
	  map_close_map(fd);
  ...
  ret = map_write_subtableheader(&header,sizeof header,fd,&head);
  if ( ret < 0 )
    {
      map_close_map(fd);
      return ret;
  ...
  status = map_overwrite_pointer(&head,sizeof head,fd,ROOT);
  if ( status < 0 )
    {
      map_close_map(fd);
      return status;
