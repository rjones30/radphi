     ...
     
{
  ...
  *nitem = 0;
  ...
  if ( header.negversion >= 0 )
	...
	{
	  pred_loc = ROOT;
	  status = map_read_pointer(&succ_loc, sizeof succ_loc, fd, pred_loc);
	  if ( status != MAP_OK ) 
	    {
	      map_close_map(fd);
	      return status;
      ...
      else
	{
	  if ( succ_loc != NULL_LOC )
	    {
	      status = map_read_subsystem(&nextsubsystem, sizeof nextsubsystem, fd, 
				          succ_loc);
	      if ( status != MAP_OK ) 
		{
		  map_close_map(fd);
		...
		{		
		  *nitem = 0;
		  ...
		  while ( pred_loc != NULL_LOC )
		    {
		      (*nitem)++;
		      status = map_read_pointer(&pred_loc,sizeof pred_loc,fd,pred_loc);
		      if ( status != MAP_OK ) 
			{
			  map_close_map(fd);
			  return status;
	  ...
	  if ( tindex < header.table_used )
	    {
	      ...
	      status = map_read_subsystem(&nextsubsystem, sizeof nextsubsystem, 
				fd, table[tindex].loc);
	      if ( status != MAP_OK ) 
		{
		  free(table);
