/* map_create_item.c 
   created Sep 7 1999 by Craig Steffen. */
...
int main(int nargs, char *args[]){
  int length;
  if(nargs!=5){
    fprintf(stdout,"usage: map_create_subsystem <mapfilename> <subsystem name> <new item> <length>\n");
    exit(1);
  }
  length=atoi(args[4]);
  ...
  if(map_add_item(args[1],args[2],args[3],length,MAP_FLOAT)){
