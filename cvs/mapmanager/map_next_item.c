  ...
  if ( subsystem.list >= 0 )
      ...
      else
	{
	  if ( succ_loc != NULL_LOC )
	    {
	      status = map_read_item(&nextitem, sizeof nextitem, fd, succ_loc);
	      if ( status != MAP_OK ) 
		{
		  map_close_map(fd);
		  return status;
	...
	table = (nametable_t *) calloc(subsystem.table_used,
				       sizeof(nametable_t));
	status = map_read_nametable(table,subsystem.table_used*sizeof(nametable_t),
			            fd,subsystem.table);
	if ( status != MAP_OK )
	  {
	    free(table);
	...
	if ( tindex < subsystem.table_used )
	  {
	    ...
	    status = map_read_item(&nextitem, sizeof nextitem, fd,
			      table[tindex].loc);
	    if ( status != MAP_OK ) 
	      {
		free(table);
