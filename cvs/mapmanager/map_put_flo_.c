...
int map_put_float_(char *filename, char *subsystemname,
 ...
 
{
	...
	map_error(MAP_USER_ERROR_ARGUMENT,
		  "MAP_PUT_FLOAT: filename longer (%d) than %d characters.",
		  filename_len,
    ...
    if ( subsystemname_len > MAP_NAMELENGTH-1 )
      {
	map_error(MAP_USER_ERROR_ARGUMENT,
		  "MAP_PUT_FLOAT: subsystemname longer (%d) than %d characters.",
		  subsystemname_len,
		  MAP_NAMELENGTH-1);
	return MAP_USER_ERROR_ARGUMENT;
      }
    ...
    if ( itemname_len > MAP_NAMELENGTH-1 )
      {
	map_error(MAP_USER_ERROR_ARGUMENT,
		  "MAP_PUT_FLOAT: itemname longer (%d) than %d characters.",
		  itemname_len,
		  MAP_NAMELENGTH-1);
	return MAP_USER_ERROR_ARGUMENT;
      }
    ...
    return map_put_float(filename_copy,subsystemname_copy,itemname_copy,
