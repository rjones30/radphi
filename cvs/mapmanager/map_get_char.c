     ...
     
{
 ...
 tryagain:
  ...
  if ( status < 0 )
    {
      map_close_map(fd);
      if ( try == 1 && status == MAP_USER_WARN_NOMATCH )
	{
	  try = 2;
	  goto tryagain;
	}
  ...
  *vtime = succ_time;
  ...
  status = map_read_arrayheader(&header,sizeof header,fd,succ_loc);
  if ( status != MAP_OK ) 
    {
      map_close_map(fd);
      return status;
  ...
  *olength = length;
  ...
  status = map_read_char(array,(*olength)*sizeof(char),fd,succ_loc+ARRAY_VALUES_OFFSET);
  if ( status != MAP_OK ) 
    {
      map_close_map(fd);
      return status;
