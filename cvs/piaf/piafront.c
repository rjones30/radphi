 ...
 *  Modified login to support only login as user piaf, with the convention
 *  that the username be entered as the password.  The default behaviour
 *  poses a security risk by passing the username/password over the network
 *  in nearly plain text (1's complement).  The modified behaviour has its
 *  own security problems: users who understand the piaf client/server
 *  protocol are able to execute arbitrary commands on the server as the
 *  piaf user.  However the piaf user has restricted rights, so this is
 *  considered a manageable risk and well worth the convenience of piaf.
 *  Comment out the next line to restore the default behaviour.
 */
#define USE_USERNAME_AS_PASSWD
...
#if !defined(__hpux) && !defined(_POSIX_SOURCE)
...
#ifdef USE_USERNAME_AS_PASSWD
  if (strcmp(user_name, "piaf") != 0) {
    sprintf(msg, "Login failed - only user %s is allowed access", "piaf");
    fatal_error(msg);
  }
  else if ((pw = getpwnam(pass_word)) == NULL ) {
    sprintf(msg, "Login failed - user %s unknown", pass_word);
    fatal_error(msg);
  }
  else if ((pw = getpwnam(user_name)) == NULL ) {
    sprintf(msg, "Login failed - user %s unknown", user_name);
    fatal_error(msg);
  }
#else
