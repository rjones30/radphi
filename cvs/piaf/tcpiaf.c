 ...
 *  Fixed a bad bug in recv_len() that failed to recognize a return from
 *  recv() with zero bytes as a EOF, as in "the slave is gone".  This fixes
 *  a bug which caused the master server under linux to hang in an endless
 *  running-loop waiting for input as soon as a slave went away.
 *  To disable this fix, comment out the following line.
 */
...
/*
 * $Id: tcpiaf.c,v 1.1.1.1 2004/03/20 20:05:12 radphi Exp $
 *
 * $Log: tcpiaf.c,v $
